...
import os, sys
import ROOT as r
from ROOT import TTree, TFile, AddressOf, gROOT, TLorentzVector
...
try:    input_file = open( sys.argv[1], 'r')
except:
...
P4_v=r.vector('ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiM4D<float>>')()
...
s = MyStruct()
event_wgt = MyFloat()
output_tree.Branch('n_particles',r.addressof(s,'n_particles'),'n_particles/I')
...
output_tree.Branch('event_wgt',r.addressof(event_wgt,'weight'),'event_wgt/F')
    ...
    if in_ev == 1:
        ...
        try:
            if line.split()[1] == "1":
                ...
                a = TLorentzVector(float(line.split()[6]), float(line.split()[7]), float(line.split()[8]), float(line.split()[9]))
                VecLep = Math.LorentzVector('ROOT::Math::PtEtaPhiM4D<float>')(a.Pt(), a.Eta(), a.Phi(), a.M())
